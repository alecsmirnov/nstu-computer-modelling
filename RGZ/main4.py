...
import distribution4 as distr4
import file_functions as ff


INPUT_PATH  = "input/"
OUTPUT_PATH = "output/task4/"

# Названия файла входных данных тестов
TESTS_FILENAME = "tests_settings4.txt"
...
def read_tests_settings(filename):
    ...
    f.close()
    return n_list, n, k, alpha, precision, histogram_run, chart_run


def main():
    # Чтение данных тестов
    n_list, n, k, alpha, precision, histogram_run, chart_run = read_tests_settings(INPUT_PATH + TESTS_FILENAME)
    # Инициализация параметров для выполнения нескольких распределений
    DISTR_COUNT    = 3
    DISTR_TITLE    = ["Стюдент", "Хи-квадрат", "Нормального"]
    DISTR_FILENAME = ["student", "chi2", "norm"]
    EMPIRIC_DISTR  = [distr4.student_distribution, distr4.chi2_distribution, distr4.normal_distribution]
    THEOR_DISTR    = [distr4.t.cdf, distr4.chi2.cdf, distr4.norm.cdf]
    ARGS_LIST      = [[n], [k], []]
    ARGS_LABEL     = [["n"], ["k"], []]
    for i in range(DISTR_COUNT):
        for j in range(len(n_list)):
            ...
            smir_S, smir_PSS, smir_passed = alg.smirnov_test(sequence, alpha, THEOR_DISTR[i], *ARGS_LIST[i])
            ff.write_tests_results(OUTPUT_PATH + ff.get_filename(DISTR_FILENAME[i], TESTS_RESULT, n_list[j]), precision, 
                                   ...
                                   chi2_r, chi2_S, chi2_PSS, chi2_passed, smir_S, smir_PSS, smir_passed)
            if histogram_run:
